...
//______________________________CP shorthand END__________________________
	...
	public static void main(String[] args) throws Exception {
		...
		for (int t = 0; t < T; t++) {
			...
			for(int i = 0; i < M ; i++){
				...
				al	= adjList.containsKey(x) ? adjList.get(x) : new ArrayList<Integer>();
				al.add(y);
				adjList.put(x,al);
				
				al	= adjList.containsKey(y) ? adjList.get(y) : new ArrayList<Integer>();
				al.add(x);
				adjList.put(y,al);
			}
			start = INT();
	...
	private static String dijkstra(Map<Integer, ArrayList<Integer>> adjList, int start) {
