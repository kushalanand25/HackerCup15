...
//		int T = INT();
		...
		SOP(PrimMST(adjList,start));
		PRINT();
	}
	
	private static int PrimMST(Map<Integer, ArrayList<Node>> adjList, int start) {
		...
		while (!que.isEmpty()) {
			Node nd 	= que.remove();
			Integer u	= nd.val;
			Collections.sort(adjList.get(u));
			if(nd.cost == Integer.MAX_VALUE>>1)
				break;
